...
<!--ADMIN SENT TO DM MODAL-->
<div id="admin_dm_modal" class="modal fade" role="dialog">
  <div class="order-modal-dialog">

  
  <?php echo form_open(URL_DM_VIEW_ORDER);?>
    <!-- Modal content-->
    <div class="modal-content order-content">
      ...
      <div class="modal-body">
		...
		<input type="hidden" name="order_id" id="admin_sent_order_id" value="">
...
<div id="km_dm_modal" class="modal fade" role="dialog">
  ...
  <?php echo form_open(URL_DM_VIEW_ORDER);?>
    ...
    <div class="modal-content order-content">
      ...
      <div class="modal-body">
		<p><?php echo get_languageword('kitchen_manager_has_been_sent_an_order');?></p>
		<p><?php echo get_languageword('would_you_like_to_view');?>?</p>
...
<script>
    ...
    channel.bind('admin_dm_event', function(data) {
		
		var ord_id= data.order_id;
		
		$('#admin_sent_order_id').val(ord_id);
		$('#admin_dm_modal').modal('show');
		playAudio();
	});
	/*ADMIN SENT TO DM*/
	...
	/*KM SENT TO DM*/
    var channel = pusher.subscribe('my-channel');
    channel.bind('km_dm_event', function(data) {
...
<?php echo $this->load->view('common/popup_script');?>	
